...
for nn=2:30
    ...
    Rc=Rc(:,1:100);Ori=Ori(:,1:100);is_in=is_in(1:100);
    % plot3(p(:,1),p(:,2),p(:,3),'.')
    Rc0=Rc;
    ...
    % pore_fix_id=find(cellfun(@(c)length(unique(c)),num2cell(Td_fix,2))==4);
    % 在多个颗粒中间的id，使用封闭的集合体需要改上一行，而且不一定能筛干净
    % find(cellfun(@(c)length(unique(c)),num2cell(Td_fix,2))==1)% 不存在颗粒内部的

    
    tic
    loop_var=Td;
    num_zero=numCore-mod(size(loop_var,1),numCore);
    spmd_cell=num2cell(reshape([1:size(loop_var,1) ones(1,num_zero)],numCore,[]),2);
    loop_times=length(spmd_cell{1});
    Cv=distributed(Cv);
    Vv=distributed(Vv);
    Rc0=distributed(Rc0);
    Ori=distributed(Ori);
    Td_fix=distributed(Td_fix);
    spmd(numCore)
        loop_list=spmd_cell(spmdSize);
        rs_pore=zeros(3,length(loop_list));
        dist_pore=zeros(1,length(loop_list));
        Nd_idx=cell(1,length(loop_list));
        for ii=1:loop_times
            %     parfor ii=1:size(Td,1)
            Cv_tmp=cell2mat(Cv(Td(ii,:))');
            idx=mode(Cv_tmp(Cv_tmp~=1));% to remove inf
            rs_pore_tmp=Vv(idx,:)';
    ...
    rc_g_cell=rc_g_cell(cellfun('length',rc_g_cell)>1);
    %     range_cell=range_cell(cellfun('length',r_sphere_cell)>1);
    %     r_sphere_cell=r_sphere_cell(cellfun('length',r_sphere_cell)>1);
    %%
    %     save(['..\basic\pack\' fileList(nn).name '\pore.mat'],rc_g_cell,range_cell,r_sphere_cell)
    save(['..\basic\pack\' fileList(nn).name '\pore.mat'],'rc_g_cell')
end