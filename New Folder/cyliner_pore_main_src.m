...
for nn=16:30
    ...
    parfor ii=1:length(IDX_merge1)% connect for the second time
        ...
        if length(pore_particle)>4% 不是在缝隙中的不需要第二次合并
            continue
        end
        ...
        if isempty(merge_neighbor)
            continue
        end
        merge_neighbor_particle=cellfun(@(c)unique(Td_fix(IDX_merge1{c},:)),num2cell(merge_neighbor),'UniformOutput',false);
        merge_neighbor=merge_neighbor(cellfun(@(c)all(ismember(pore_particle,c)),merge_neighbor_particle));% 向大的方向合并
    ...
    save(['..\basic\pack\' fileList(nn).name '\pore.mat'],'rc_g_cell','IDX_merge2','Td_fix')
end