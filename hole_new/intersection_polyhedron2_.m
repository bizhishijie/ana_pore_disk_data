function [V_inter,rc_inter_all] = intersection_polyhedron2_ (K1,K2,rc1,rc2)
...
rc_inter_line=[];
for ii=1:size(idx_face2,1)
    n_vec=cross(rc2(:,idx_face2(ii,1))-rc2(:,idx_face2(ii,2)),rc2(:,idx_face2(ii,3))-rc2(:,idx_face2(ii,2)));
    side=dot(rc1-rc2(:,idx_face2(ii,1)),repmat(n_vec,1,size(rc1,2)));
    idx_line1_tmp=idx_line1(side(idx_line1(:,1)).*side(idx_line1(:,2))<0,:);
    for jj=1:size(idx_line1_tmp,1)
        rc_inter_tmp=intersection_line_face_ (rc1(:,idx_line1_tmp(jj,:)),rc2(:,idx_face2(ii,:)));
        rc_inter_line=[rc_inter_line rc_inter_tmp];
    end
end
for ii=1:size(idx_face1,1)
    n_vec=cross(rc1(:,idx_face1(ii,1))-rc1(:,idx_face1(ii,2)),rc1(:,idx_face1(ii,3))-rc1(:,idx_face1(ii,2)));
    side=dot(rc2-rc1(:,idx_face1(ii,1)),repmat(n_vec,1,size(rc2,2)));
    idx_line2_tmp=idx_line2(side(idx_line2(:,1)).*side(idx_line2(:,2))<0,:);
    for jj=1:size(idx_line2_tmp,1)
        rc_inter_tmp=intersection_line_face_ (rc2(:,idx_line2_tmp(jj,:)),rc1(:,idx_face1(ii,:)));
        rc_inter_line=[rc_inter_line rc_inter_tmp];
    end
end
...
%         rc_inter_tmp=intersection_line_face_ (rc1(:,idx_line1(ii,:)),rc2(:,idx_face2(jj,:)));
%         rc_inter_line=[rc_inter_line rc_inter_tmp];
%     end
% end
% for ii=1:size(idx_line2,1)
%     for jj=1:size(idx_face1,1)
%         rc_inter_tmp=intersection_line_face_ (rc2(:,idx_line2(ii,:)),rc1(:,idx_face1(jj,:)));
%         rc_inter_line=[rc_inter_line rc_inter_tmp];
%     end
% end
...
if ~isempty(rc_inter_all)
    try
        ...
        %         disp('Approximated')
        try
            [~,V_inter]=convhulln(rc_inter_all',{'QJ'});
        catch
            V_inter=0;
        end
    end
