...
for ii=1:length(fileList)
    ...
    parfor line_id=1:1e4% 改变循环次数
        ...
        if size(cross_p_list,2)<4% 交点太少了就重新随机，只和一个圆盘相交算不了
            continue
        end
        ...
        cross_p_close_id=find(abs(sum(cross_p_list(4:6,3:2:end).*cross_p_list(4:6,2:2:end-2)))>0.995&...
            dot(cross_p_list(1:3,3:2:end)-cross_p_list(1:3,2:2:end-2),cross_p_list(4:6,3:2:end))<h/4);
