...
dis=zeros(1,size(cylinder_Rc,2));
...
case_4=RP_z_length<h/2&RP_r_length<r;

dis(:,case_1)=sqrt((RP_z_length(case_1)-h/2).^2+(RP_r_length(case_1)-r).^2);
dis(:,case_2)=RP_z_length(case_2)-h/2;
dis(:,case_3)=RP_r_length(case_3)-r;
dis(:,case_4)=max(RP_r_length(case_4)-r,RP_z_length(case_4)-h/2);
