clear;clc
load('p_cylinder_surface.mat');p=p';n=length(p);
...
for nn=1:15
    ...
    parfor ii=1:length(Td)
        ...
        if isempty(rs_pore_list)||~is_point_in_tetra(rs_pore_list,tetra_tmp(1,:),tetra_tmp(2,:),tetra_tmp(3,:),tetra_tmp(4,:))
            % 如果外心不在四面体的内部
            ...
            if ~isempty(dist_pore_list)
                ...
                for jj=1:size(Li_map,1)
                    ...
                    is_in_diff_disk(jj)=length(unique(Td_fix(ii,Li_map(jj,:))))>1;% 跨颗粒的边
                end
                dist_pore_list=dist_pore_list(is_in_diff_disk);
                rs_pore_list=rs_pore_list(is_in_diff_disk,:);
                [dist_pore(ii),max_idx]=max(-dist_pore_list);
                rs_pore(ii,:)=rs_pore_list(max_idx,:);
            end
        else
    ...
    parfor ii=1:length(Td)
        ...
        if particle_num==4||particle_num==3% 如果是在4个颗粒中间的，邻居也是4个的话判断互相在对方球心内
            ...
            flag2=cellfun(@(c)length(unique(c))<3,num2cell(Td_fix(Nd_tmp_loop,:),2));
    ...
    while flag
        ...
        merge_particle_cell=cell(1,length(IDX_merge2));
    ...
    load(['../basic/data/' fileList(nn).name '/edge.mat'])
    ...
    save(['../basic/data/' fileList(nn).name '/pore_data.mat'],'rc_g_cell','idx_dcell_cell')
end