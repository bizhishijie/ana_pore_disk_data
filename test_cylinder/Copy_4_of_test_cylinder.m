...
for nn=1:15
    ...
    parfor ii=1:length(Td)
        ...
        if ~isempty(rs_pore_list1)% 四点共面找不到外心
            dist_pore_list1=min(distance_point_cylinder(rs_pore_list1',Rc,Ori,cylinder_size));
        ...
        for jj=1:size(Td_map,1)
            ...
            rs_pore_list2(jj,:)=C;
            dist_pore_list2(jj)=min(distance_point_cylinder(C',Rc,Ori,cylinder_size));
        ...
        for jj=1:size(Li_map,1)
            ...
            [min_point_position,dist_pore_list3(jj)]=fminbnd(@(t)f(t,line_tmp,Rc,Ori),0,1);
    ...
    for ii=1:length(IDX_merge2)
        ...
        % plot3(rc_g(1,:),rc_g(2,:),rc_g(3,:),'.');axis equal
        rc_g_cell{ii}=rc_g;
    ...
    rc_g_cell=rc_g_cell(cell_in);
    idx_dcell_cell=idx_dcell_cell(cell_in);
